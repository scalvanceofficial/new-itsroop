...
(function ($) {
 ...
 -------------------------------------------------------------------------*/
  var totalPriceVariant = function () {
    ...
    var symbol = window.currencyConfig ? window.currencyConfig.symbol : "₹";
    var priceText = $(".price-on-sale").text().replace(symbol, "").replace(/,/g, "").trim();
    var basePrice = parseFloat($(".price-on-sale").data("base-price")) || parseFloat(priceText);
    ...
    $(".color-btn, .size-btn").on("click", function () {
      ...
      quantityInput.val(1);
      $(".price-on-sale").text(symbol + newPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ","));
      var totalPrice = newPrice;
      $(".total-price").text(symbol + totalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ","));
    });
    ...
    function updateTotalPrice() {
      var curPriceText = $(".price-on-sale").text().replace(symbol, "").replace(/,/g, "").trim();
      var currentPrice = parseFloat(curPriceText);
...
------------------------------------------------------------------------------------- */
  ...
  var totalVariant = function () {
    $(".tf-variant-item,.tf-cart-item").each(function () {
      ...
      var symbol = window.currencyConfig ? window.currencyConfig.symbol : "₹";
      var priceText = productItem.find(".tf-variant-item-price .price").text().replace(symbol, "").replace(/,/g, "").trim();
      var basePrice = parseFloat(productItem.find(".tf-variant-item-price .price").data("base-price")) || parseFloat(priceText);
      ...
      productItem.find(".color-btn, .size-btn").on("click", function () {
        ...
        productItem.find(".tf-variant-item-price .price").text(
          symbol + newPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        );
      ...
      function updateTotalPrice(price, scope) {
        var curPriceText = scope.find(".tf-variant-item-price .price").text().replace(symbol, "").replace(/,/g, "").trim();
        var currentPrice = price || parseFloat(curPriceText);
        var quantity = parseInt(scope.find("input").val());
        var totalPrice = currentPrice * quantity;
        scope.find(".tf-variant-item-total .price").text(
          symbol + totalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
        );
      }
    });
  };
