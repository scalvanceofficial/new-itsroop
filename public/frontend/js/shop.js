  ...
  var filterProducts = function () {
    ...
    function applyFilters() {
      ...
      $(".wrapper-shop .card-product").each(function () {
        ...
        const symbol = window.currencyConfig ? window.currencyConfig.symbol : "$";
        const priceText = product.find(".current-price").text().replace(symbol, "").replace(/,/g, "");
    ...
    function applyFilter(sortValue, isListActive) {
      ...
      if (sortValue === "price-low-high") {
        products.sort(
          (a, b) => {
            const symbol = window.currencyConfig ? window.currencyConfig.symbol : "$";
            return parseFloat($(a).find(".current-price").text().replace(symbol, "").replace(/,/g, "")) -
            parseFloat($(b).find(".current-price").text().replace(symbol, "").replace(/,/g, ""));
          }
        );
        ...
        products.sort(
          (a, b) => {
            const symbol = window.currencyConfig ? window.currencyConfig.symbol : "$";
            return parseFloat($(b).find(".current-price").text().replace(symbol, "").replace(/,/g, "")) -
            parseFloat($(a).find(".current-price").text().replace(symbol, "").replace(/,/g, ""));
          }
        );
